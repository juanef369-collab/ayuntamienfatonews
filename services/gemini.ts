...
export const generateNewsFromVoice = async (transcript: string): Promise<NewsArticle> => {
  ...
  const response = await ai.models.generateContent({
    ...
    contents: `Convierte este chisme en una noticia de portada satírica para el diario corporativo AyuntamienFatoNews: "${transcript}". Además, genera 3 comentarios de compañeros de oficina ficticios que reaccionen a la noticia de forma divertida. Asegúrate de que el ambiente sea puramente de oficina o mundo corporativo municipal.`,
    config: {
      responseMimeType: "application/json",
      responseSchema: NEWS_SCHEMA,
      systemInstruction: "Eres el Editor Jefe de AyuntamienFatoNews. Escribe siempre en castellano de España con jerga de oficina moderna, burocracia absurda y humor ácido."
    }
  });
...
export const generateBreakingNews = async (): Promise<NewsArticle> => {
  ...
  const response = await ai.models.generateContent({
    model: 'gemini-3-flash-preview',
    contents: "Genera una noticia de última hora sobre un desastre gracioso estrictamente relacionado con la burocracia, ayuntamientos o la oficina (máquinas de vending, reuniones de Zoom infinitas) y 3 comentarios.",
    config: {
      responseMimeType: "application/json",
      responseSchema: NEWS_SCHEMA
    }
...
export const generateImage = async (prompt: string): Promise<string> => {
  try {
    const ai = getAI();
    const imageStylePrompt = `A satirical, funny, cinematic editorial illustration for a modern newspaper, set in a messy municipal office or corporate cubicle. High quality, detailed satirical cartoon style. Scene description: ${prompt}`;
